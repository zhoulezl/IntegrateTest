...
def find_false():
    marking_result_list = {}
    with open(r'api_jmeter\openmind\model\basetest\20240709.xml', 'r', encoding='utf-8') as f:
        xml_data = f.read()
    if xml_data:
        ...
        for result in result_list:
            ...
            if '#text' in result['queryString'].keys():
                ...
                marking_result_list[marking_result[1] + marking_result[0] + 'error_msg'] = marking_result[-2].replace(r'\n', '  ')
                print(marking_result[-1])
                marking_result_list[marking_result[1] + marking_result[0] + 'args'] = marking_result[-1].replace(r'\n', '  ')

            else:
                marking_result_list[marking_result[1] + marking_result[0] + 'error_msg'] = marking_result[-1].replace(r'\n', '  ')
