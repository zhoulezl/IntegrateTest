...
from Auto_test_ly.autoTest.get_all_data import make_data_list, save_xlsx_test
from Auto_test_ly.autoTest.jmx_creater import data_create
...
if __name__ == "__main__":
    ...
    if qq.login():
        ...
        qq.addUploadFile(f"{date.tm_year}年{date.tm_mon}月{date.tm_mday}日接口自动化测试执行报告.xlsx",
                         fr"test_records/{date.tm_year}年{date.tm_mon}月{date.tm_mday}日接口自动化测试执行报告.xlsx")
        qq.send()
